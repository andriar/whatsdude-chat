  ...
  try {
    ...
    if (existingFiles && existingFiles.length > 0) {
      ...
      const deletePromises = existingFiles.map(file =>
        supabase.storage
          .from('avatars')
          .remove([`${user.value.id}/${file.name}`])
      )
      ...
      <UForm :state="{ name }" @submit.prevent="onSubmit">
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col items-center gap-2">
            ...
            <UInput type="file" accept="image/jpeg,image/png,image/gif,image/webp" @change="handleFileSelect"
              class="w-full" :disabled="avatarLoading" />
          ...
          <!-- Name Input -->
          <UInput v-model="name" type="text" placeholder="Enter your name" class="w-full" />
        </div>
